...
public class MegaFlowFumeFX
{
	...
	static public void Parse(MegaFlowFrame flow, BinaryReader br)
	{
		...
		flow.gridDim2[0] = flow.gridDim[0];
		flow.gridDim2[1] = flow.gridDim[1];
		flow.gridDim2[2] = flow.gridDim[2];
		...
		if ( (flow.flags & 0x20) != 0 )
			...
			Debug.LogWarning("No Velocity Data in FXD file, no flow imported, please ensure FXD file has velocity data present.");
